...
> 生产版本：2026 年 1–6 月｜最后整理：2026-08-05  
...
截至2026-08-05，2026H1全量预处理和OMS/DMS匹配均已完成，六份正式结果已生成；
原始发票已按类型以及PBC业务范围完成两套拆分。当前版本可供项目组和Audit复核，
但跨期Not Test、客户IT缺失单据原因、ZA05最终AQPP范围及DMS币种/数量单位仍待确认；OMS币种一致性已由财务确认。
...
| `tolerance_utils.py` | 金额/数量容差、渠道边界差异与浮点保护的唯一实现 |
...
| 配置 | 当前值 | 判断 |
|---|---:|---|
| 金额容差 | 0.02 | OMS：`abs(a-b) < 0.02`；DMS AQPP：`abs(a-b) <= 0.02` |
| 数量容差 | 0.02 | `abs(a-b) < 0.02` |
| FY25 尾差阈值 | 1.00 | 去年“尾差<1”细分 |

所有容差集中在 `config.py`，主流程不得直接用浮点数 `a == b`。代码通过
`tolerance_utils.py`执行判断，并使用`1e-9`浮点保护带，确保`10.02-10.00`
稳定识别为0.02边界。DMS AQPP将该边界作为可接受金额尾差，
数量三方一致时进入AQPP-01，并在明细列`尾差0.02`标记为“是”；OMS仍按严格小于0.02判断。
为保持FY25展示口径，DMS的0.02边界在`去年格式汇总`仍归入
`2.数量一致金额有差异 / 2.1 尾差<1`，不计入该Sheet的`1.完全匹配`。
...
- 财务已确认OMS订单、发运与发票货币种类一致，因此OMS的USD等非本位币在订单/发运币种缺失时标记“财务确认一致”，可以进入AQPP；
- 上述确认不外推到DMS；DMS非本位币缺订单/发运币种时仍进入NT-00；
...
OMS明细统一使用四个标准展示字段：`开票金额`=`实际金额（ZFN1）`、`发票含税金额`=`含税金额`、
`发票不含税金额`=`无税金额`、`开票数量`=`开票数量（基本单位数量）`。这些字段在聚合记录和
关键键缺失的仅发票记录中采用同一映射；其中只有`开票金额`参与OMS AQPP金额判断，其余字段只用于复核。
...
OMS `Not Test`只展示上述标准发票金额、税额、数量和SAP发票号列，不再并列保留
`实际金额（ZFN1）`、`含税金额`、`无税金额`、`开票数量（基本单位数量）`等原始重复列。
当前42,476条记录中34,486条具有发票金额，标准列合计为：开票金额11,369,091.34、
发票含税金额11,369,091.34、发票不含税金额10,064,584.54、开票数量-33,069.14；
其余7,990条无发票记录显示N/A，不参与上述合计。原14条USD NT-00已按财务确认转入AQPP-01。
...
34项小样本测试覆盖AQPP 24组、OMS严格边界及财务确认币种准入、DMS金额`<=0.02`及FY25尾差展示、币种/单位冲突、全额/部分/跨期冲销、
...
最新OMS及DMS全量匹配均已于2026-08-05完成。快照如下：

| 渠道 | 汇总记录数 | AQPP小计 | Not Test | AQPP+Not Test发票净额 | 完全匹配记录数 | 完全匹配金额 |
|---|---:|---:|---:|---:|---:|---:|
| DMS | 537,931 | 500,855 | 37,076 | 3,129,945,595.55 | 499,383 | 3,078,066,297.94 |
| OMS | 45,521 | 3,045 | 42,476 | 398,135,961.46 | 3,043 | 386,061,736.17 |

OMS各公司汇总中保留7,990条`公司代码缺失`、金额0的无发票记录：NT-28 2,084条、
NT-29 13条、NT-31 5,893条。其余具有发票公司/销售组织的记录已按行正确回退映射。

与昨晚尚未将政策排除发票纳入Not Test、且DMS的0.02边界尚未纳入AQPP-01的版本相比，
报告范围净额及完全匹配发票金额占比如下：

| 渠道 | 完全匹配发票金额 | 昨晚版本分母 | 昨晚占比 | 当前分母 | 当前占比 | 提升百分点 | 相对提升 |
|---|---:|---:|---:|---:|---:|---:|---:|
| DMS | 3,078,066,297.94 | 3,151,890,431.95 | 97.60% | 3,129,945,595.55 | 98.34% | 0.74 | 0.76% |
| OMS | 386,061,736.17 | 412,988,318.52 | 93.35% | 398,135,961.46 | 96.97% | 3.61 | 3.87% |

DMS当前完全匹配金额含351条0.02尾差、金额1,837,516.18元；OMS当前完全匹配金额另含14条经财务确认币种一致的USD记录、金额525,755元。其余占比变化来自政策排除负数净额纳入Not Test及OMS业务范围修正。
