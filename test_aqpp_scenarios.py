...
class AqppScenarioTests(unittest.TestCase):
    ...
    def test_currency_and_unit_assumptions_are_explicit(self):
        source = pd.DataFrame({
        ...
        self.assertEqual(measured.iloc[1]['币种校验状态'], '待确认-非本位币缺少订单发运币种')
        self.assertTrue(result.iloc[0]['AQPP可分类'])
        self.assertFalse(result.iloc[1]['AQPP可分类'])
        self.assertEqual(result.iloc[1]['AQPP场景编码'], 'NT-00')
